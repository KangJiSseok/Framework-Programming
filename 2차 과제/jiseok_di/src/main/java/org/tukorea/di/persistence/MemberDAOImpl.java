...
public class MemberDAOImpl implements MemberDAO {
    ...
    public StudentVO read(String id) throws EmptyResultDataAccessException {
    ...
    public void updateStudent(StudentVO student) throws Exception {
        ...
        StudentVO vo = jdbcTemplate.queryForObject("SELECT * FROM STUDENT WHERE ID=?", new BeanPropertyRowMapper<StudentVO>(StudentVO.class), student.getId());

        jdbcTemplate.update(
                "UPDATE STUDENT SET passwd = ?, username = ?, snum = ?, depart = ?, mobile = ?, email = ? WHERE id = ?",
                student.getPasswd(), student.getUsername(), student.getSnum(), student.getDepart(), student.getMobile(), student.getEmail(), student.getId()
        );
    }
}
